...
func appsodyImageStreamTest(t *testing.T, f *framework.Framework, ctx *framework.TestCtx) error {
	...
	// Get the application
	f.Client.Get(goctx.TODO(), types.NamespacedName{Name: name, Namespace: ns}, appsody)
	if err != nil {
		t.Fatal(err)
	}

	firstImage := appsody.Status.ImageReference
	...
	secondImage := appsody.Status.ImageReference
